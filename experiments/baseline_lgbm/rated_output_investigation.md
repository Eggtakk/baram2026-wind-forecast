...
시도한 4가지(saturation/power-curve feature, 앙상블, 모델출력 보정, 예보입력
보정) 중 saturation/power-curve만 전체 점수에 소폭 기여했고, 나머지는
효과가 없거나 불안정했다. 정격출력 구간의 오차는 예보 입력의 구조적
불확실성(분산 증가)에서 오는 것으로 보이며, 같은 입력 데이터를 재가공하는
접근으로는 한계에 도달한 것으로 판단. 이 구간을 더 파고들기보다는
하이퍼파라미터 탐색 확대나 다른 그룹3 업무로 리소스를 옮기는 것을 권장.
