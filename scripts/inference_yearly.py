"""
scripts/train_yearly.py로 학습한 모델(연 단위 holdout으로 재검증한 레시피/파라미터)로
test 기간을 예측해 제출 파일을 만든다.

실행: (레포 루트에서, scripts/train_yearly.py를 먼저 실행한 뒤) python3 scripts/inference_yearly.py
출력: submissions/lgbm_yearly_submission.csv
"""
import json
import sys
...
OUT_PATH = SUBMISSION_DIR / "lgbm_yearly_submission.csv"
...
def predict_group(group_id: int) -> pd.DataFrame:
    meta_path = MODEL_DIR / f"group{group_id}_yearly_meta.json"
    model_path = MODEL_DIR / f"group{group_id}_yearly_model.pkl"
    if not model_path.exists():
        raise FileNotFoundError(f"{model_path} 가 없습니다. 먼저 scripts/train_yearly.py를 실행하세요.")
    ...
    if recipe == "physics":
        ...
        curve_path = MODEL_DIR / f"group{group_id}_yearly_power_curve.pkl"
