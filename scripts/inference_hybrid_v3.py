"""
34번 섹션 최종 — group1은 quantile objective stage-1(alpha=0.60)+잔차보정
그대로 유지, group2는 FICR objective를 T=0.025/lambda_l2=0.5로 재튜닝한
버전(ficr_v2)+잔차보정(threshold=0.65)으로 교체(LOYO delta+0.0280,
기존 배포본 +0.0266보다 우세하고 baseline std를 처음으로 넘음), group3는
기존 프로덕션 그대로(31번 섹션에서 quantile+잔차보정이 LOYO 통과하고도
실 제출에서 뒤집혀 채택 보류 중).

실행: (레포 루트에서, 아래가 먼저 실행되어 있어야 함)
  train_final_ficr_v2.py, train_residual_stage_production_ficr_v2.py
출력: submissions/lgbm_hybrid_v3_submission.csv
"""
import json
import sys
...
OUT_PATH = SUBMISSION_DIR / "lgbm_hybrid_v3_submission.csv"

STAGE_SUFFIX = {1: "quantile", 2: "ficr_v2"}
...
def predict_group_with_residual(group_id: int) -> pd.DataFrame:
    """group1/2용 — suffix(quantile/ficr_v2)에 맞는 stage-1+잔차보정."""
...
def main():
    ...
    if prev_path.exists():
        ...
        for gid in [1, 2, 3]:
            ...
            print(f"  {col}: 직전 배포본(group2 ficr-v1) 대비 평균|delta|={diff.mean():.2f} kWh, "
