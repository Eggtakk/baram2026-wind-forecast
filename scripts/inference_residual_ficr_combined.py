"""
group1+group2에 FICR-shaped custom objective stage-1(25번 섹션) + 정격출력
2단계 잔차보정을 함께 적용한 제출 파일 생성 (group3는 기존 프로덕션 그대로).
`scripts/inference_residual_combined.py`(L2 stage-1 + 잔차보정, 이미 실 제출해
0.61778 확정)의 FICR objective 버전.

실행: (레포 루트에서, train_final_ficr.py + train_residual_stage_production_ficr.py
1/2가 이미 실행되어 있어야 함)
  python3 scripts/inference_residual_ficr_combined.py
출력: submissions/lgbm_group12_residual_ficr_submission.csv
"""
import json
import sys
...
OUT_PATH = SUBMISSION_DIR / "lgbm_group12_residual_ficr_submission.csv"

FICR_GROUPS = {1, 2}  # group3는 기존 프로덕션(L2 stage-1, 잔차보정 없음) 그대로
...
def predict_group_baseline(group_id: int) -> pd.DataFrame:
    """group3용 — 기존 L2 stage-1, 잔차보정 없음 (inference_final.py와 동일)."""
...
def predict_group_ficr_with_residual(group_id: int) -> pd.DataFrame:
    """group1/2용 — FICR objective stage-1 + 잔차보정."""
    meta_path = MODEL_DIR / f"group{group_id}_final_ficr_meta.json"
    with open(meta_path, "r", encoding="utf-8") as f:
        stage1_meta = json.load(f)
    stage1_model = joblib.load(MODEL_DIR / f"group{group_id}_final_ficr_model.pkl")
    feature_cols = stage1_meta["feature_cols"]
    recipe = stage1_meta["recipe"]

    residual_meta_path = MODEL_DIR / f"group{group_id}_residual_stage_ficr_meta.json"
    with open(residual_meta_path, "r", encoding="utf-8") as f:
        residual_meta = json.load(f)
    stage2_model = joblib.load(MODEL_DIR / f"group{group_id}_residual_stage_ficr_model.pkl")
    ...
    if recipe == "physics":
        ...
        df = build_baseline_features(df)
        curve_models = load_power_curve_models(MODEL_DIR / f"group{group_id}_final_ficr_power_curve.pkl")
        df = apply_power_curve_models(df, curve_models)
    ...
    print(f"[group{group_id}] FICR stage-1 예측 기준 regime(threshold={threshold}) 행 수: "
...
def main():
    ...
    for gid in [1, 2, 3]:
        pred_df = predict_group_ficr_with_residual(gid) if gid in FICR_GROUPS else predict_group_baseline(gid)
    ...
    # 직전 제출본(L2 stage-1 + 잔차보정, group1+group2) 대비 비교
    prev_path = SUBMISSION_DIR / "lgbm_group12_residual_submission.csv"
    if prev_path.exists():
        prev = pd.read_csv(prev_path, parse_dates=["forecast_kst_dtm"])
        for gid in [1, 2, 3]:
            col = f"kpx_group_{gid}"
            diff = (submission[col] - prev[col]).abs()
            print(f"  {col}: 직전 제출(L2+잔차) 대비 평균|delta|={diff.mean():.2f} kWh, "
