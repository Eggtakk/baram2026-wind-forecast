"""
group2만 정격출력 2단계 잔차 보정을 적용한 제출 파일 생성.
group1/3은 scripts/inference_final.py와 완전히 동일(기존 프로덕션 모델 그대로).

실행: (레포 루트에서, train_final.py + train_group2_residual_stage.py가
이미 실행되어 있어야 함)
  python3 scripts/inference_group2_residual.py
출력: submissions/lgbm_group2_residual_submission.csv
"""
import json
import sys
...
OUT_PATH = SUBMISSION_DIR / "lgbm_group2_residual_submission.csv"
...
def predict_group_baseline(group_id: int) -> pd.DataFrame:
    """scripts/inference_final.py와 동일 — 프로덕션 모델 그대로 사용."""
...
def predict_group2_with_residual() -> pd.DataFrame:
    group_id = 2
    meta_path = MODEL_DIR / f"group{group_id}_final_meta.json"
    with open(meta_path, "r", encoding="utf-8") as f:
    ...
    print(f"[group{group_id}] 보정 전/후 평균 발전량: "
          ...
          f"(regime 구간 평균 보정폭: {(final_pred[regime_mask]-stage1_pred[regime_mask]).mean():.1f} kWh)"
          if n_regime > 0 else "")
...
def main():
    ...
    for gid in [1, 2, 3]:
        pred_df = predict_group2_with_residual() if gid == 2 else predict_group_baseline(gid)
    ...
    # 기존 프로덕션 제출본과 비교
    prod_path = SUBMISSION_DIR / "lgbm_final_submission.csv"
    if prod_path.exists():
        prod = pd.read_csv(prod_path, parse_dates=["forecast_kst_dtm"])
        for gid in [1, 2, 3]:
            col = f"kpx_group_{gid}"
            diff = (submission[col] - prod[col]).abs()
            print(f"  {col}: 기존 대비 평균|delta|={diff.mean():.2f} kWh, "
                  f"최대|delta|={diff.max():.2f} kWh, delta!=0인 행 수={int((diff > 1e-6).sum())}")
