"""
group3 quantile+잔차보정 파이프라인 검증 후 최종본 — 3개 그룹 전부 각자
최적인 stage-1 objective로 구성:
  group1: quantile objective(alpha=0.60) + 잔차보정 (30번 섹션에서 교체 확정)
  group2: FICR-shaped objective(alpha 없음, ficr_weight=0.008) + 잔차보정 (28번 섹션 배포본 유지)
  group3: quantile objective(alpha=0.65) + 잔차보정 (신규 — LOYO delta+0.0172,
    baseline std 0.0098를 크게 초과. FICR objective는 noise 수준(+0.002)에
    그쳐 채택하지 않음)

`scripts/inference_hybrid_quantile_ficr.py`(group3는 기존 프로덕션 그대로)의
후속 버전 — group3까지 잔차보정 파이프라인으로 승격.

실행: (레포 루트에서, 아래가 먼저 실행되어 있어야 함)
  train_final_quantile.py 1, train_residual_stage_production_quantile.py 1
  train_final_quantile.py 3, train_residual_stage_production_quantile.py 3
  (group2는 기존 _ficr 파일 그대로 재사용)
  python3 scripts/inference_hybrid_all3.py
출력: submissions/lgbm_hybrid_all3_submission.csv
"""
import json
import sys
...
OUT_PATH = SUBMISSION_DIR / "lgbm_hybrid_all3_submission.csv"

# group_id -> stage-1/stage-2 파일 suffix ("quantile" 또는 "ficr")
STAGE_SUFFIX = {1: "quantile", 2: "ficr", 3: "quantile"}
...
def predict_group_with_residual(group_id: int) -> pd.DataFrame:
    """전 그룹 공용 — suffix(quantile/ficr)에 맞는 stage-1+잔차보정."""
...
def main():
    ...
    for gid in [1, 2, 3]:
        pred_df = predict_group_with_residual(gid)
    ...
    prev_path = SUBMISSION_DIR / "lgbm_hybrid_quantile_ficr_submission.csv"
    if prev_path.exists():
        prev = pd.read_csv(prev_path, parse_dates=["forecast_kst_dtm"])
        for gid in [1, 2, 3]:
            col = f"kpx_group_{gid}"
            diff = (submission[col] - prev[col]).abs()
            print(f"  {col}: 직전 제출(group3 미교체본) 대비 평균|delta|={diff.mean():.2f} kWh, "
