"""
scripts/train_final2.py로 학습한 최종 블렌드 모델로 test 기간을 예측해
제출 파일을 만든다.

실행: (레포 루트에서, scripts/train_final2.py를 먼저 실행한 뒤) python3 scripts/inference_final2.py
출력: submissions/lgbm_final2_blend_submission.csv
"""
import json
import sys
...
OUT_PATH = SUBMISSION_DIR / "lgbm_final2_blend_submission.csv"
...
def predict_group(group_id: int) -> pd.DataFrame:
    meta_path = MODEL_DIR / f"group{group_id}_final2_meta.json"
    if not meta_path.exists():
        raise FileNotFoundError(f"{meta_path} 가 없습니다. 먼저 scripts/train_final2.py를 실행하세요.")
    ...
    if recipe == "physics":
        ...
        curve_path = MODEL_DIR / f"group{group_id}_final2_power_curve.pkl"
    ...
    blended = None
    for name, weight in meta["blend"]:
        model = joblib.load(MODEL_DIR / f"group{group_id}_final2_{name}.pkl")
        pred = model.predict(df[feature_cols])
        blended = pred * weight if blended is None else blended + pred * weight

    blended = blended.clip(min=0, max=capacity)
    return pd.DataFrame({"forecast_kst_dtm": df["forecast_kst_dtm"], f"kpx_group_{group_id}": blended})
