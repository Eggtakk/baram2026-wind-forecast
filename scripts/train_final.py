...
+0.005~0.009 개선을 예측했으나, 실제 제출 결과 리더보드 총점이 0.6088로
오히려 하락(-0.0015)해 기각. 원인 추정: 낮은 풍속 임계값(6~7m/s)이 2024년
holdout에만 존재하는 커틀먼트/착빙 이벤트 패턴에 과적합되어, 정상적인
저-중풍속 시간대까지 학습 데이터에서 과도하게 제거한 것으로 보임. 상세는
experiments/baseline_lgbm/rated_output_investigation.md 8번 섹션 참고.
이 스크립트는 그 실패를 반영해 원래(0.30/8.0, 기존 파라미터 소스)로
되돌린 상태.
