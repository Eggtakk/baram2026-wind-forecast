"""
30번 섹션 — quantile objective stage-1(scripts/train_final_quantile.py) 위에
정격출력 2단계 잔차보정을 다시 얹을 때 threshold/stage2_params를 처음부터
재탐색한다. `scripts/tune_residual_stage_ficr.py`(FICR objective 버전)와
병렬 구조.

stage-1이 quantile로 바뀌면 잔차(residual = y - oof_pred)의 부호/분포
자체가 L2·FICR 때와는 질적으로 달라진다(quantile 회귀는 의도적으로
한쪽으로 치우친 예측을 하므로, "정격출력 구간에서 과소예측된 만큼만 보정"
하던 기존 threshold/파라미터가 그대로 맞을 이유가 없음) — 그래서 이번엔
재사용 없이 threshold 그리드 + stage2 Optuna를 처음부터 다시 돌린다.

실행 (레포 루트에서):
  python3 scripts/tune_residual_stage_quantile.py prepare <group_id> [year ...]
  python3 scripts/tune_residual_stage_quantile.py grid <group_id> [threshold ...]
  python3 scripts/tune_residual_stage_quantile.py search <group_id> <threshold> [budget_sec]
  python3 scripts/tune_residual_stage_quantile.py best <group_id> <threshold>
  python3 scripts/tune_residual_stage_quantile.py validate <group_id>
캐시: /tmp/residual_stage_quantile_cache_group{gid}.pkl
결과: experiments/baseline_lgbm/loyo_residual_stage_quantile_results.json
"""
import json
import pickle
import sys
...
STORAGE = f"sqlite:///{LOCAL_DB_DIR / 'residual_optuna_quantile.db'}"
SEED = 42
OOF_SPLITS = 3
...
RESULTS_PATH = OUT_DIR / "loyo_residual_stage_quantile_results.json"
BEST_CONFIG_PATH_TMPL = str(OUT_DIR / "group{gid}_residual_stage_quantile_best_config.json")


def cache_path(group_id: int) -> Path:
    return CACHE_DIR / f"residual_stage_quantile_cache_group{group_id}.pkl"
...
def prepare(group_id: int, holdout_years=None):
    ...
    for holdout_year in years:
        ...
        print(f"  fold holdout={holdout_year}: quantile-stage1-only score={baseline_score:.4f} "
...
def grid_search(group_id: int, thresholds=None):
    ...
    print(f"\n=== group{group_id} threshold 그리드서치 (quantile stage-1 단독 mean={baseline_scores.mean():.4f}) ===")
    default_stage2 = dict(
...
def search(group_id: int, threshold: float, budget_seconds: int = 30):
    ...
    cache = load_cache(group_id)
    study_name = f"group{group_id}_residual_thr{threshold:.2f}_quantile"
...
def best(group_id: int, threshold: float):
    ...
    config = {"threshold": threshold, "stage2_params": b.params, "quantile_alpha": QUANTILE_ALPHA[group_id]}
...
def validate(group_id: int):
    ...
    for year, fold in folds.items():
        ...
        fold_results.append({
            ...
            "quantile_stage1_only_score": fold["baseline_score"],
            "threshold": threshold, "n_regime_train": n_regime,
        })
        print(f"  holdout={year}: combined={score:.4f} quantile_stage1_only={fold['baseline_score']:.4f} "
              f"delta_vs_L2_baseline={score-l2_base_mean:+.4f}")

    scores = np.array([r["score"] for r in fold_results])
    q_only = np.array([r["quantile_stage1_only_score"] for r in fold_results])
    print(f"\n=== group{group_id} 최종 요약 ===")
    print(f"  L2 baseline: mean={l2_base_mean:.4f}")
    print(f"  quantile stage-1 단독: mean={q_only.mean():.4f} delta={q_only.mean()-l2_base_mean:+.4f}")
    print(f"  quantile stage-1 + 잔차보정: mean={scores.mean():.4f} delta={scores.mean()-l2_base_mean:+.4f} "
